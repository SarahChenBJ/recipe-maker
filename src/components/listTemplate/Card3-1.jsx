...
//vertical: img + ungroup
export const ViewCard_ungroup_ver_img = ({ data }) => {
    const downloadCard = () => {
        ...
        <div className='instr-text card-serve'>SERVES {data.serves}</div>
                <div className='card-instruction'>
                    ...
                    <div className='step-item step instr-text'>
                        {data.instructions[0].steps && data.instructions[0].steps.map((s) => (<div className='vbstep-item'>{s}</div>))}
                    </div>
                </div>
                </div>
            </div>
